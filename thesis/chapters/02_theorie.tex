...
Es ist bereits länger bekannt, dass es sich beim Minimum-Bisection-Problem um ein $NP$\hyp schweres Problem handelt.~\parencite{gj79}
In jüngerer Vergangenheit wurde von Feige und Krauthgamer~\parencite{fk02} ein polynomieller Algorithmus präsentiert, der das Problem mit einem polylogarithmischen Approximationsfaktor löst. 
...
Diese Behauptung wird im folgenden Satz, der aus \parencite{ar06} entnommen ist, gezeigt. 
...
\section{Ein bikriterieller Graphpartitionierungsalgorithmus}
Wie im vorherigem Abschnitt gezeigt wurde, ist es nicht möglich, das $(k,1)$\hyp Partitionierungsproblem in polynomieller Zeit zu approximieren. 
Deshalb ist ein bikriterieller Ansatz sinnvoll, der das Problem in Hinsicht auf die Ausgeglichenheit der Partitionierung und in Hinsicht auf die Schnittkosten approximiert.
...
Für diese Umwandlung wird ein Verfahren verwendet, das von Räcke~\parencite{rc08} entwickelt wurde. 
...
\begin{defn}[Signatur]\label{defn:signature}
    Sei $\calS$ eine Menge von Zusammenhangskomponenten über der Knotenmenge $V$ von $T$ und $\eps > 0$. Ein Vektor $\vec{g} = (g_0, \ldots, g_t)$ mit $t = \ceil*{\log_{1+\eps}(1/\eps)} + 1$ wird Signatur von $\calS$ genannt, wenn $\calS$ genau $g_0$ Komponenten mit Größe in $\big[1,\, \eps \ceil*{n/k}\big)$ und $g_i$ Komponenten mit Größe in $\big[{(1+\eps)}^{i-1}\, \cdot\, \eps \ceil*{n/k},\, {(1+\eps)}^i\, \cdot \, \eps \ceil*{n/k}\big)$ für $i \in \{1, \ldots, t\}$ enthält.
\end{defn}
...
Im ersten Schritt wird dynamische Programmierung verwendet, um die Signaturen und die zugehörigen Repräsentanten zu finden.
Dabei wird einen Ansatz genutzt, der eine Verallgemeinerung des Ansatzes für das Bisektionsproblem auf Bäumen ist. \parencite{mcg78, ws11}
...
Hierbei handelt es sich um das Bin-Packing-Problem, wofür von Hochbaum und Shmoys~\parencite{hs86} ein Approximationsalgorithmus vorgestellt wurde.
...
Dieses Verfahren wurde zuerst von Feldmann und Foschini~\parencite{FF15} vorgestellt und ist wiederum eine Verallgemeinerung der Lösungsmethode für das Bisektionsproblem auf Bäumen. \parencite{mcg78, ws11}
...
Nun wird gezeigt, dass sich $C_v(\vec{g}, m)$ durch ein dynamisches Programm berechnen lässt.

Sei $\mathcal{F}^*$ die optimale untere Grenze im Bezug auf $C_v(\vec{g}, m)$. Dann ergeben sich vier Fälle, die daraus resultieren, dass ein Knoten entweder kein rechtestes Kind hat oder nicht und dass ein Knoten entweder einen linken Geschwisterknoten hat oder nicht.
...
Sonst sind die Schnittkosten $\weight(e)$, wobei $e$ die Kante von $v$ zu seinem Elternknoten ist, da für die untere Grenze $\mathcal{F}^*$ gilt, dass $V \setminus \mathcal{F}^*$ eine Komponente darstellt und damit $e$ von der Komponente $\{v\}$ zur Komponente $V \setminus \mathcal{F}^*$ geht.
Also gilt $C_v((0,\ldots,0), 0) = 0$ und $C_v(\vec{e}(1), 1) = \weight(e)$ und alle anderen Funktionswerte sind unendlich.

Der zweite Fall ist, dass $v$ weder ein Blatt noch der linkeste Knoten unter seinen Geschwisterknoten ist. Sei $w$ im Folgenden der Vorgänger von $v$ im Bezug auf die Geschwisterknoten von $v$ und sei $u$ das rechteste Kind von $v$. 
...
Jetzt kann $\mathcal{F}^*$ entweder eine untere Grenze sein, in der die Kante $e$ von $v$ zu seinem Elternknoten geschnitten wird oder nicht.
Für beide Fälle ist die Visualisierung in Abbildung~\ref{fig:lower_frontier} hilfreich, welche weitgehend unverändert aus~\cite{FF15} übernommen wurde.
...
Nun kann es noch sein, dass $v$ kein Blatt ist und $v$ einen linken Geschwisterknoten hat, aber die Kante von $v$ zu seinem Elternknoten geschnitten wird.
...
\begin{figure}
    \centering
    \includestandalone{figures/misc/lower_frontier}
    \caption{Teilbaum mit einem Knoten $v$ und dessen rechten Kind $u$ und linken Geschwisterknoten $v$ in dem $m$ Knoten der Menge $L_v$ durch eine untere Grenze (in schwarz) überdeckt werden\label{fig:lower_frontier}}
\end{figure}

Somit kann $C_v(\vec{g}, m)$ für den Fall, dass $v$ sowohl ein Kind als auch einen linken Geschwisterknoten hat, berechnet werden, indem das Minimum der durch~\eqref{eq:e_not_cut} und~\eqref{eq:e_cut} gegebenen Werte bestimmt wird.
...
Der folgende Satz von Feldmann und Foschini~\parencite{FF15} zeigt, dass die oben genannten rekursiven Definitionen von $C_v(\cdot, \cdot)$ verwenden werden können, um mit dynamischer Programmierung die Lösungsmenge $\bbS$ in polynomieller Zeit für konstantes $\eps$ zu berechnen.
...
\begin{proof}
    ...
    \end{equation}
    berechnet werden.
    Die Menge von Zusammenhangskomponenten $\calS$ mit der Signatur $\vec{g}$ kann wiederum rekursiv mit Hilfe von dynamischer Programmierung unter Verwendung der obigen rekursiven Formeln für $C_v$ berechnet werden.
    ...
    Deshalb kann man $g_i$ mit 
    \begin{equation*}
        \frac{\card{L_v}}{{(1 + \eps)}^{i - 1} \cdot \eps n/k} \leq \frac{k}{{(1 + \eps)}^{i - 1} \cdot \eps}
    \end{equation*}
        für $i \in \{1, \ldots, t\}$ beschränken und mit $\card{L_v}$ für den Fall $i = 0$.
    ...
    Schlussendlich steht fest, dass die Anzahl der Signaturen an einem Knoten $v$ gleich $\gamma \card{L_v}$ ist, wobei $\gamma \in \bigO \left({(k/\sqrt{\eps})}^{1 + \ceil{1/\eps \cdot \log(1/\eps)}}\right)$.
    ...
    Wir stellen die Behauptung auf, dass $T_v \leq \frac{3}{2} \gamma^2 \card{L_v}^4$ für jeden Knoten $v$ gilt.
...
Außerdem reduziert dieser Rundungsschritt die Anzahl der verschiedenen Komponentengrößen auf $t = \ceil*{\log_{1+\eps}(1/\eps)} + 1$, was ein Konstante für konstantes $\eps$ ist.
...
Insgesamt ist die Laufzeit dieses Schritts also $\bigO\big({(k/\eps)}^{2t}\big)$.
...
Wenn in keinem der Behälter Platz ist, dann wird ein neuer Behälter hinzugefügt und die Komponente in diesen gepackt.
Dieser Schritt kann in Zeit $\bigO(n^2)$ durchgeführt werden, da $g_0 \leq n$ gilt und es höchstens $n$ Behälter geben kann.
Zusammenfassend lässt sich sagen, dass für eine gegeben Menge von Zusammenhangskomponenten $\calS$ mit Signatur $\vec{g}$ in Zeit $\bigO\big({(k/\eps)}^{2t} + n^2\big)$ geprüft werden kann, ob $\calS$ für eine zulässige $(k, 1 + \eps)$\hyp Partitionierung verwendet werden kann.
...
\begin{lem}\label{lem:bincnt}
    ...
    Für eine Menge $\calS \in \bbS$ mit Signatur $\vec{g}^*$ erhält man $\varphi(\calS) \leq k$.
\end{lem}
\begin{proof}
...
\end{proof}
    ...
    Im nächsten Satz, der aus~\parencite{FF15} stammt, wird die Korrektheit und die Laufzeit des Algorithmus gezeigt.

    \begin{thm}\label{thm:treealg}
        Für einen Baum mit positiven Kantengewichten und Parameter $\eps > 0$ und $k \in \{1, \ldots, n\}$ existiert ein Algorithmus, der eine Partitionierung der Knoten von $T$ in $k$ Mengen berechnet, sodass jede der $k$ Mengen höchstens eine Größe von $(1 + \eps) \ceil*{n/k}$ hat und die Schnittkosten höchstens den Schnittkosten einer optimalen perfekt ausgewogenen Partitionierung des Baums entsprechen. 
...
Dabei hat der präsentierte Algorithmus für generelle Graphen $G$ mit Kantengewichten einen Approximationsfaktor von $\bigO(\log n)$, das heißt, die vom Algorithmus gefundene Partitionierung hat Schnittkosten, die höchstens um einen Faktor in $\bigO(\log n)$ von den optimalen Schnittkosten einer perfekt balancierten Partitionierung entfernt sind.
...
Im Folgenden werden nur die nötigen grundlegenden Konzepte von Dekompositionsbäumen~\parencite{rc08} eingeführt.
...
Deswegen kann mit einer leicht veränderten Version des durch Satz~\ref{thm:treealg} gegebenen Algorithmus eine $(k,1 + \eps)$\hyp Partitionierung berechnet werden, deren Schnittkosten nur um einen Faktor $\bigO(\log n)$ von den optimalen Schnittkosten der $(k, 1)$\hyp Partitionierung entfernt ist.
...
Die Schnittkosten $C(\calL)$ einer Partitionierung $\calL$ ist dann das minimale Gewicht der Kanten, die entfernt werden müssen, um die Mengen $L_1, \ldots, L_k$ voneinander zu trennen.
...
\begin{cor}\label{cor:leafpartitioning}
    Für einen Baum $T$ mit positiven Kantengewichten und Parameter $\eps > 0$ und $k \in \{1, \ldots, n\}$ existiert ein Algorithmus, der eine Partitionierung der $l$ Blätter von $T$ in $k$ Mengen berechnet, sodass jede der $k$ Mengen höchstens $(1 + \eps) \ceil*{l/k}$ Blätter enthält und die Schnittkosten höchstens den Schnittkosten einer optimalen perfekt ausgewogenen Blattpartitionierung des Baums entsprechen. 
        Weiterhin ist die Laufzeit für konstantes $\eps$ polynomiell.
\end{cor}

Mit diesem Ergebnis kann nun das Hauptergebnis von~\parencite{FF15} bewiesen werden, welches zeigt, dass fast ausgeglichene Partitionierungen in generellen Graphen berechnet werden können, die um einen logarithmischen Faktor vom Optimum entfernt sind.
...
\begin{proof}
    ...
    Für jeden dieser Bäume wird der durch Korollar~\ref{cor:leafpartitioning} gegebene Algorithmus verwendet, um eine Partitionierung der Blätter in $k$ Mengen mit Größe höchstens $(1 + \eps) \ceil*{n/k}$ zu berechnen. Dann wird die Blattpartitionierung $\calL^*$ aus dem Dekompositionsbaum $T^*$ ausgewählt, welche die Schnittkosten des entsprechenden Schnitts in $G$ minimiert.
