...
Für den Graph $G$ ist ein Dekompositionsbaum ein Baum $T=(V_T, E_T, \weight_T)$ mit Gewichtsfunktion $\weight_T : E_T \rightarrow \reals^+$, dessen Blätter $L \subset V_T$ bijektiv auf die Knoten $V$ von $G$ abgebildet werden können.
...
Deswegen kann mit einer leicht veränderten Version des durch~\ref{thm:treealg} gegebenen Algorithmus eine $(k,1 + \eps)$-Partitionierung erreicht werden, deren Schnittkosten nur um einen Faktor $\bigO(\log n)$ von den optimalen Schnittkosten der $(k, 1)$-Partitionierung entfernt ist.
...
\begin{proof}
    ...
    Wenn nun die Optimallösung $\calV^* = \{ V_1^*, \ldots, V_k^* \}$ des $(k, 1)$-Partitionierungsproblems betrachtet wird, dann folgt für die dazugehörige Blattpartitionierung $m_{T^\prime}(\calV^*) = \{ m_{T^\prime}(L_1), \ldots, m_{T^\prime}(L_k) \}$ die obere Schranke $C(m_{T^\prime}(\calV^*)) \leq \bigO(\log n) \cdot C(\calV^*)$.
