...
Um zu überprufen, ob eine Menge von Zusammenhangskomponenten $\mathcal{S}$ für eine gültige $(k,1+\eps)$-Partitionierung des Graphen verwendet werden kann, versucht der Algorithmus in der zweiten Phase die Komponenten in $\mathcal{S}$ in $k$ Behälter der Größe $(1 + \eps) \ceil*{n/k}$ zu packen.
...
Dieser Algorithmus wird im Weiteren beschrieben.
Zuerst entfernen wir alle Komponenten in $\mathcal{S}$ mit einer Größe kleiner oder gleich $\eps \ceil*{n/k}$ und lösen dann eine Bin-Packing-Instanz ohne diese kleinen Komponenten.
